...
template<typename T>
struct SegTree {
    int n; vector<T> t; T ID;                  
    function<T(T,T)> F;                        
 
    SegTree(int n, T ID, function<T(T,T)> F)
    ...
    void upd(int v,int l,int r,int i,T val){
    ...
    // Range query: [L, R]
    T qry(int v,int l,int r,int L,int R) const {
    ...
    T query(int L,int R) const { return qry(1,0,n-1,L,R); }
};
...
//	cin>>t;
	...
	for(int i=0;i<1;i++)
	{
		...
		SegTree<int> st(n, INT_MAX, [](int a, int b){ return min(a,b); });
		st.build(a);
