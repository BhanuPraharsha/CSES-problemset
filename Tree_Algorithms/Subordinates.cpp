...
void dfs(int u, int parent, vector<int> &ans, vector<vector<int>> &adjlst)
{
    for(int j=0;j<adjlst[u].size();j++)
    {
        if(adjlst[u][j]==parent) continue;
        dfs(adjlst[u][j], u, ans, adjlst);
        ans[u]+=ans[adjlst[u][j]]+1;
    }
...
int main() {
    ...
    for(int j=2;j<=n;j++)
    {
        int temp;
        cin>>temp;
        adjlst[j].push_back(temp);
        adjlst[temp].push_back(j);
    }
